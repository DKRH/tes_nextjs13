...
export const Add = () => {
  ...
  const url = "/api/era/data-path/dkrh";
  const method = "POST";

  return mainForm(fname, fdesc, btnName, submitBtnName, title, url, method);
};

export const Update = ({ paths }: { paths: DataPath }) => {
  const fname = paths.name;
  const fdesc = paths.desc;
  const btnName = "Edit";
  const submitBtnName = "Update";
  const title = "Update Path";
  const url = `/api/era/data-path/${paths.id}`;
  const method = "PATCH";

  return mainForm(fname, fdesc, btnName, submitBtnName, title, url, method);
};

export const Delete = ({ paths }: { paths: DataPath }) => {
  const [isOpen, setIsOpen] = useState(false);
  ...
  const handleDelete = async (id: number) => {
    setIsLoading(true);
    await fetch(`/api/era/data-path/${id}`, {
      method: "DELETE",
      cache: "no-store",
    });
    setIsLoading(false);
