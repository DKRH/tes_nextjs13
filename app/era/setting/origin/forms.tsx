...
export const Add = () => {
  ...
  const url = "/api/era/data-origin/dkrh";
  const method = "POST";

  return mainForm(fname, fdesc, btnName, submitBtnName, title, url, method);
};

export const Update = ({ origins }: { origins: DataOrigin }) => {
  const fname = origins.name;
  const fdesc = origins.desc;
  const btnName = "Edit";
  const submitBtnName = "Update";
  const title = "Update Path";
  const url = `/api/era/data-origin/${origins.id}`;
  const method = "PATCH";

  return mainForm(fname, fdesc, btnName, submitBtnName, title, url, method);
};

export const Delete = ({ origins }: { origins: DataOrigin }) => {
  const [isOpen, setIsOpen] = useState(false);
  ...
  const handleDelete = async (id: number) => {
    setIsLoading(true);
    await fetch(`/api/era/data-origin/${id}`, {
      method: "DELETE",
      cache: "no-store",
    });
    setIsLoading(false);
      ...
      <div className={isOpen ? "modal modal-open" : "modal"}>
        <div className="modal-box">
          <h3 className="font-bold text-lg">
            Are sure to delete {origins.name}?
          </h3>
          ...
          <div className="modal-action">
            ...
            {!isLoading ? (
              <button
                type="button"
                onClick={() => handleDelete(origins.id)}
